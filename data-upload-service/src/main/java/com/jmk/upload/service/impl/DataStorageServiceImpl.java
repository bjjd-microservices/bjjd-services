...
public class DataStorageServiceImpl implements DataStorageService{
	...
	public Map<String,List<? extends Object>> storeData(Map<String, List<? extends Base>> sheetMapping) {
		...
		sheetMapping.entrySet().stream().forEach(entry->{
			...
			if(entry.getKey().equals("Expense")) {
				ResponseEntity<List<Expense>> responseEntity=accountMgmtServiceClient.saveExpenses(entry.getValue().stream().map(expense->modelMapper.map(expense,Expense.class)).collect(Collectors.toList()));
				resultSheetMapping.put("Expense", responseEntity.getBody());
			}
		});
